...
float3 applyColorGrade(float3 color, constant ClipGradeParameters& grade) {
    for (int i = 0; i < grade.count; ++i) {
        ...
        switch (grade.entries[i].kind) {
            case 0: color = applyExposure(color, amount); break;
            case 1: color = applyContrast(color, amount); break;
            case 2: color = applySaturation(color, amount); break;
            case 3: color = applyVibrance(color, amount); break;
            case 4: color = applyTemperature(color, amount); break;
            case 5: color = applyTint(color, amount); break;
            case 6: color = applyHighlights(color, amount); break;
            case 7: color = applyShadows(color, amount); break;
            default: break;
        }
    }
    return color;
...
fragment float4 fragment_working(VertexOut in [[stage_in]],
                              ...
                              constant ClipGradeParameters& grade [[buffer(1)]]) {
    float2 codedUV;
